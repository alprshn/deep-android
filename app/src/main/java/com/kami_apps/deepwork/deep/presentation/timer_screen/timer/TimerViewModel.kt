package com.kami_apps.deepwork.deep.presentation.timer_screen.timer
...
import com.kami_apps.deepwork.deep.data.manager.TimerManager
    ...
    fun completeSession() {
        ...
        if (tagId == 0) {
            viewModelScope.launch(Dispatchers.IO) {
                // yine de timer’ı sıfırla ve AppBlocking’i durdur
                timerManager.reset()
                stopAppBlocking()
            }
            Log.d("TimerViewModel", "Session completed with NO TAG — not saving to DB.")
            return
        }
        viewModelScope.launch(Dispatchers.IO) {
            timerManager.reset()
